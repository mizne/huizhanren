...
export class SmsEffects {
  @Effect()
  preSendSms$ = this.actions$.ofType(fromSms.PRE_SEND_SMS)
  ...
  .map(([templateId, customers]) => {
    const phones = customers.reduce((accu, curr) => (accu.push(...curr.phones), accu), [])
    if (phones.length === 0) {
  ...
  toSendSms$ = this.actions$.ofType(fromSms.TO_SEND_SMS)
  ...
  @Effect({ dispatch: false })
  cancelSendSms$ = this.actions$.ofType(fromSms.CANCEL_SEND_SMS)
  .do(() => {
    this.toastCtrl.create({
      message: '取消发送短信',
      duration: 3e3,
  ...
  @Effect()
  ensureSendSms$ = this.actions$.ofType(fromSms.ENSURE_SEND_SMS)
  .map((action: fromSms.EnsureSendSMSAction) => action.templateId)
  .withLatestFrom(this.store.select(getSmsTemplates), (templateId, templates) => templates.find(e => e.id === templateId))
  .withLatestFrom(this.store.select(getPhonesToSendOfCustomers), (template, customers) => ({template, customers}))
  .withLatestFrom(this.store.select(getSelectedExhibitionId), ({template, customers}, exhibitionId) => ({template, customers, exhibitionId}))
  .mergeMap(({template, customers, exhibitionId}) => {
    const customerPhones = customers.map(e => e.phones.map(f => ({phone: f, name: e.customerName})))
    ...
    .concatMap(() => {
      ...
      return [
        ...
        new fromLogger.BatchCreateLoggerAction({customerIds: customers.map(e => e.customerId), log: {
          level: 'sys',
          content: `系统: 发送 【${template.label}】 短信成功!`
        }}),
        new fromSms.MarkCustomerHasSendSMSAction(customers.map(e => e.customerId)),
  ...
  @Effect()
  markCustomerHasSendSms$ = this.actions$.ofType(fromSms.MARK_CUSTOMER_HAS_SEND_SMS)
  ...
  @Effect()
  markCustomerHasSendSmsSuccess$ = this.actions$.ofType(fromSms.MARK_CUSTOMER_HAS_SEND_SMS_SUCCESS)
  .map(() => new fromCustomer.FetchAllAction())


  @Effect({ dispatch: false })
  sendSmsSuccess$ = this.actions$.ofType(fromSms.SEND_SMS_SUCCESS, fromSms.SINGLE_SEND_SMS_SUCCESS).do(() => {
    this.toastCtrl.create({
      message: '发送短信成功',
      duration: 3e3,
      position: 'top'
  ...
  @Effect({ dispatch: false })
  sendSmsFailure$ = this.actions$.ofType(fromSms.SEND_SMS_FAILURE, fromSms.SINGLE_SEND_SMS_FAILURE).do(() => {
    this.toastCtrl
      .create({
        message: '发送短信失败',
        duration: 3e3,
  ...
  @Effect({ dispatch: false })
  toSingleSendSMS$ = this.actions$.ofType(fromSms.TO_SINGLE_SEND_SMS)
  ...
  @Effect()
  ensureSingleSendSMS$ = this.actions$.ofType(fromSms.ENSURE_SINGLE_SEND_SMS)
  .map((action: fromSms.EnsureSingleSendSMSAction) => action.payload)
  .withLatestFrom(this.store.select(getShowDetailCustomerId), ({templateId, phone}, customerId) => ({templateId, phone, customerId}))
  .withLatestFrom(this.store.select(getCustomers), ({templateId, phone, customerId}, customers) => {
  ...
  .withLatestFrom(this.store.select(getSmsTemplates), ({templateId, phone, customer}, templates) => {
  ...
  .withLatestFrom(this.store.select(getSelectedExhibitionId), ({template, phone, customer}, exhibitionId) => ({
    template,
    phone,
    customer,
    exhibitionId
  ...
  @Effect()
  fetchAllTemplate$ = this.actions$.ofType(fromSms.FETCH_ALL_TEMPLATE)
  .mergeMap(() =>
    this.smsService
      .fetchAllTemplate()
  ...
  @Effect({ dispatch: false })
  fetchAllTemplateSuccess$ = this.actions$.ofType(fromSms.FETCH_ALL_TEMPLATE_SUCCESS).do(() => {
    // this.toastCtrl.create({
    //   message: '获取短信模板列表成功',
    //   duration: 3e3,
    //   position: 'top'
  ...
  @Effect({ dispatch: false })
  fetchAllTemplateFailure$ = this.actions$.ofType(fromSms.FETCH_ALL_TEMPLATE_FAILURE).do(() => {
    // this.toastCtrl
    //   .create({
    //     message: '获取短信模板列表失败',
    //     duration: 3e3,

...
export function shouldFireScrollEvent(
  ...
  if (scrollingDown) {
    remaining = (container.totalToScroll - container.scrolledUntilNow - container.height) / container.totalToScroll;
    containerBreakpoint = distance.down / 10;
  } else {
    remaining = (container.scrolledUntilNow + container.height) / container.totalToScroll;
